    ...
    <Card>
      <div class="bg-black">
        <nav class=" flex justify-between w-[98%] mx-auto">
          ...
          <div
            id="nav"
            class="md:static md:min-h-fit absolute min-h-[30vh] left-0 top-[-100%] md:w-auto w-full flex items-right px-5 bg-black"
          >
            <ul class="flex md:flex-row flex-col md:items-center  md:gap-[4vw] gap-2">
              <div class="flex flex-row">
                {menuClick && (
                  <AccessibilityOutline color={"#ffffff"} width="13px" />
                )}{" "}
                <a class={navButton} onClick={handleClick}>
                  About
                </a>
              </div>
              <div class="flex flex-row">
                {menuClick && <OptionsOutline color={"#ffffff"} width="13px" />}{" "}
                <a class={navButton} onClick={handleSkillClick}>
                  Skills
                </a>
              </div>
              <div class="flex flex-row">
                {menuClick && (
                  <CodeSlashOutline color={"#ffffff"} width="13px" />
                )}{" "}
                <a
                  class={navButton}
              ...
              <div class="flex flex-row">
                {menuClick && (
                  <ConstructOutline color={"#ffffff"} width="13px" />
                )}{" "}
                <a class={navButton} onClick={handleProjectClick}>
                  Projects
                </a>
              </div>
              <div class="flex flex-row">
                {menuClick && <PeopleOutline color={"#ffffff"} width="13px" />}{" "}
                <a
                  class={navButton}
              ...
              <div class="flex flex-row">
                {menuClick && (
                  <MailUnreadOutline color={"#ffffff"} width="13px" />
                )}{" "}
                <a class={navButton} href="mailto:joshuaruthless@gmail.com">
                  Email
                </a>
              </div>
              <div class="flex flex-row">
                {menuClick && (
                  <DocumentTextOutline color={"#ffffff"} width="13px" />
                )}{" "}
                <a
                  class={navButton}
                  href={CV}
          ...
          <div class="cursor-pointer md:hidden flex mr-4 mt-4">
            {menuClick ? (
              <CloseOutline color={"white"} onClick={closeClicked} />
            ) : (
              <ListOutline color={"#ffffff"} onClick={menuClicked} />
            )}
          </div>
        </nav>
      </div>
